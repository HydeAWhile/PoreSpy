...
3. Then you create a virtual environment in that directory using ``uv venv --python 3.12``. This adds a ``.venv`` folder, where ``uv`` will store all its information. Note that PoreSpy only supports python version 3.11, 3.12 and 3.13. This is largely because of dependencies which have firm version requirements.
